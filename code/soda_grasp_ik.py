...
def createBw():
    ...
    #     [0, 0],
    #     [0, 0],
    #     [-0.03, 0.03], # Keeping range to increase tolerance; can set to [0, 0] if need to sample TSRs strictly at the same height
    #     [0, 0],
    #     [0, 0],
    #     [-np.pi, np.pi]
    # ])
...
def main(if_sim):
    ...
    for configuration in configurations:
        ...
        # build to find path
        trajectory = planner.build()

        # get out of loop when trajectory is found
        if trajectory:
            break
    ...
    close_hand(hand, preshape=(1.0, 1.0))
    ###
    ...
    # # Q6
    # delta_x = np.array([0, 0, 0, -desired_height, 0, 0])
    # J = arm_skeleton.get_jacobian(hand.get_endeffector_body_node())
    # delta_q_err = np.matmul(np.linalg.pinv(J), delta_x)
    # q = arm_skeleton.get_positions() - delta_q_err
    ...
    # Q7
    step_size = 0.01
    n_iter = int(desired_height / step_size)

    for _ in range(n_iter):
        delta_x = np.array([0, 0, 0, -step_size, 0, 0])
        J = arm_skeleton.get_jacobian(hand.get_endeffector_body_node())
        delta_q_err = np.matmul(np.linalg.pinv(J), delta_x)
        q = arm_skeleton.get_positions() - delta_q_err
