...
class AdaRRT():
    ...
    def build(self):
        ...
        for k in range(self.max_iter):
            ...
            nearest_neighbor = self._get_nearest_neighbor(random_sample)
            new_node = self._extend_sample(random_sample, nearest_neighbor)
    ...
    def _get_random_sample(self):
        ...
        :returns: A vector representing a randomly sampled point in the search
            space.
        """
        random_sample = [np.random.uniform(lower, upper) for lower, upper in zip(self.joint_lower_limits, self.joint_upper_limits)]
        return random_sample

    def _get_random_sample_near_goal(self, distance=0.05):
        """
        Uniformly samples around the goal within the given distance along each
            axis of the search space.

        :returns: A vector representing a randomly sampled point around the
            goal within the given distance in the search space.
        """
        offset = np.random.uniform(-distance, distance, size = len(self.goal.state))
        random_sample_near_goal = self.goal.state + offset
    ...
    def _extend_sample(self, sample, neighbor):
        ...
        new_sample = neighbor.state + self.step_size * unit_direction
        
        is_in_collision = self._check_for_collision(new_sample)
        if not is_in_collision:
            new_node = neighbor.add_child(new_sample)
            return new_node
